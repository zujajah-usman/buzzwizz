	      ...
	      <section class="content">
	        <div class="row">
	            ...
	            <?php
	       			...
	       			$row = $stmt->fetch();
	       			if($row['numrows'] < 1){
	       				echo '<h1 class="page-header">No results found for <i>'.escape_html($keyword).'</i></h1>';
	       			}
	       			else{
	       				echo '<h1 class="page-header">Search results for <i>'.escape_html($keyword).'</i></h1>';
		       			try{
		       			 	$inc = 3;	
						    ...
						    foreach ($stmt as $row) {
						    	$highlighted = preg_filter('/' . preg_quote($keyword, '/') . '/i', '<b>$0</b>', escape_html($row['name']));
							...
							echo "Error loading search results.";
						}
					}
