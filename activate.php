...
<?php
	...
	if(!isset($_GET['code']) OR !isset($_GET['user'])){
		...
		$user_id = validate_int($_GET['user'] ?? 0);
		
		if (empty($code) || $user_id === false) {
			$output .= '
				<div class="alert alert-danger">
	                <h4><i class="icon fa fa-warning"></i> Error!</h4>
	                Invalid activation code or user ID.
	            </div>
	            <h4>You may <a href="signup.php">Signup</a> or back to <a href="index.php">Homepage</a>.</h4>
			';
		} else {
			...
			if($row['status']){
				...
				$output .= '
					<div class="alert alert-success">
		                ...
		                Account activated - Email: <b>'.escape_html($row['email']).'</b>.
		            </div>
		            <h4>You may <a href="login.php">Login</a> or back to <a href="index.php">Homepage</a>.</h4>
				';
				}
					...
					$output .= '
						<div class="alert alert-danger">
			                <h4><i class="icon fa fa-warning"></i> Error!</h4>
			                Unable to activate account. Please try again later.
			            </div>
			            <h4>You may <a href="signup.php">Signup</a> or back to <a href="index.php">Homepage</a>.</h4>
					';
				}
