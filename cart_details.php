	...
	if(isset($_SESSION['user'])){
		...
		try{
			...
			foreach($stmt as $row){
				...
				$output .= "
					<tr>
						<td><button type='button' data-id='".escape_html($row['cartid'])."' class='btn btn-danger btn-flat cart_delete'><i class='fa fa-remove'></i></button></td>
						<td><img src='".escape_html($image)."' width='30px' height='30px'></td>
		...
		if(count($_SESSION['cart']) != 0){
			...
			foreach($_SESSION['cart'] as $row){
				...
				$output .= "
					<tr>
						<td><button type='button' data-id='".escape_html($row['productid'])."' class='btn btn-danger btn-flat cart_delete'><i class='fa fa-remove'></i></button></td>
						<td><img src='".escape_html($image)."' width='30px' height='30px'></td>
